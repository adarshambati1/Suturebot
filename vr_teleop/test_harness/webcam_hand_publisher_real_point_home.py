"""Webcam hand teleop for the REAL Rizon4s ("Titania") -- point/pinch + home button.

Same as webcam_hand_publisher_real_point.py (clutched relative position +
orientation, pointing pose to move, pinch to toggle grip) PLUS:

  * On startup, the arm's current pose is captured as HOME.
  * Pressing 'h' anytime commands the arm back to that captured pose (position
    AND orientation). Hand-driven streaming is auto-disengaged at the same
    moment so the goal you just wrote isn't immediately overridden -- press
    'e' to resume teleop after the arm settles.
...
Orientation, position clutch, axis mapping, smoothing, depth-bbox subset, and
safety all match webcam_hand_publisher_real_point.py.

Usage:
    python vr_teleop/test_harness/webcam_hand_publisher_real_point_home.py --port 6380
"""
...
ORI_MIN_CUTOFF = 0.4    # original: 0.8
ORI_BETA = 0.1
...
def draw_hud(frame, engaged, active, pos, grip_mode, fps, hand_seen, clamped, ori_on, rot_deg, homing):
    h, w = frame.shape[:2]
    if homing:
    ...
    bot = (f"e: engage   point: move+rotate   pinch: toggle grip   h: home   r: recenter   q: quit   "
           f"fps={fps:4.1f}   hand={'yes' if hand_seen else 'no '}")
    cv2.putText(frame, bot, (10, h - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200, 200, 200), 1)
...
def main() -> None:
    ...
    # Capture HOME: the arm's pose at this moment becomes the recall target.
    ...
    WINDOW = "Suturebot webcam teleop (REAL: point/pinch + home)"
    ...
    print(f"REAL ROBOT teleop (point/pinch + home). Redis {args.host}:{args.port}, robot={ROBOT_NAME}")
    ...
    print("Keys: 'e' engage   point=move+rotate   pinch=toggle grip   'h' return home   'r' recenter   'q' quit")
    ...
    rot_deg = 0.0
    homing_until = 0.0   # show "RETURNING TO HOME" banner for a brief moment after press
    ...
    def go_home() -> None:
        """Command the captured home pose; disengage so hand-streaming doesn't override."""
    ...
    while True:
        ...
        draw_hud(frame, engaged, active, last_pos, grip_mode, fps, hand_seen, clamped, ori_on, rot_deg, homing_banner)
        ...
        if key == ord('h'):
            go_home()
            homing_until = time.monotonic() + 2.0   # banner for 2 s
