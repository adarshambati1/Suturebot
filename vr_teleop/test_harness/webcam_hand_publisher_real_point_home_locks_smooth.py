"""Webcam hand teleop for REAL Rizon4s -- point/pinch + home + locks + decoupled publisher.

Same controls as webcam_hand_publisher_real_point_home_locks.py, but the
Redis writes for goal_position and goal_orientation are decoupled from
MediaPipe's frame rate by a background thread. The thread publishes
interpolated goals to Redis at ~100 Hz so the controller always sees a
smoothly-changing target -- no more pulsing/pausing as discrete MediaPipe
frames arrive every ~50 ms.

How the decoupling works:
  * Main loop (~20 fps): MediaPipe -> One-Euro filter -> target_pos & R_goal_ori.
    Each frame, the main loop calls publisher.update(target_pos, R_goal_ori).
  * Publisher thread (~100 Hz): linearly interpolates position and SLERPs
    orientation from the last segment's start to the latest target as time
    advances, writing the interpolated value to Redis each tick.
  * The interpolation interval auto-adapts to the observed MediaPipe gap.
  * The thread is only active while engaged + active + hand_seen; when
    disengaged/inactive it stops writing so other clients can take over.

Controls (same as the locks file):
    'e'      -> engaged / disengaged
    point    -> active (curl middle/ring/pinky) -> move + rotate
    pinch    -> toggle gripper open<->grip
    'r'      -> recenter while active
    'h'      -> return to home pose captured at startup
    x/y/z    -> lock translation in X / Y / Z
    X/Y/Z    -> lock rotation about X / Y / Z (roll / pitch / yaw)
    'q'      -> quit

Usage:
    python vr_teleop/test_harness/webcam_hand_publisher_real_point_home_locks_smooth.py --port 6380
"""
...
EURO_MIN_CUTOFF = 0.5     # moderated default (the heavily-extreme combo
EURO_BETA = 1.0           # caused pulsing on small motion -- tune at runtime)
AXIS_CUTOFF_SCALE = np.array([0.6, 0.3, 1.0])

DEADBAND_M = 0.0
NO_HAND_TIMEOUT = 1.0

PUB_RATE_HZ = 100.0       # background publisher rate
SMOOTH_TAU = 0.12         # PT1 time constant (s) layered on top of interpolation
                          # to round velocity-vector corners between segments.
                          # 0 disables; higher = smoother + laggier.
...
class TargetPublisher(threading.Thread):
    """Background thread that publishes goal_position/goal_orientation at a
    fixed rate, interpolating between successive main-loop updates.

    The main loop calls update(pos, ori) at MediaPipe's ~20 Hz frame rate;
    this thread fills the gaps by linearly interpolating position and SLERPing
    orientation toward the latest target, writing to Redis at PUB_RATE_HZ.
    This eliminates the pulse-and-pause feel from discrete frame-rate writes.
    """

    INIT_INTERVAL = 0.05    # initial guess for inter-update interval (s)
    STALE_GAP = 0.5         # treat the next update as a fresh segment if the
                            # previous one was >= this long ago (no jump)
    EMA_ALPHA = 0.8         # smoothing on the inter-update interval estimate

    def __init__(self, r: redis.Redis, k_pos: str, k_ori: str, rate_hz: float,
        ...
        self._prev_ori: np.ndarray | None = None
        self._target_ori: np.ndarray | None = None
        self._t_set = 0.0
        self._interval = self.INIT_INTERVAL
        # PT1 stage on top of interpolation -- rounds corners between segments.
        self._pub_pos: np.ndarray | None = None
        self._pub_ori: np.ndarray | None = None
        self._t_pub = 0.0
    ...
    def reset(self) -> None:
        """Forget any pending segment. Use when the source changes (e.g. on
        clutch reanchor or home recall) to avoid interpolation jumps."""
        with self._lock:
            self._prev_pos = None
            self._target_pos = None
            self._prev_ori = None
            self._target_ori = None
            self._t_set = 0.0
            self._interval = self.INIT_INTERVAL
            self._pub_pos = None
            self._pub_ori = None
    ...
    def update(self, pos: np.ndarray, ori: np.ndarray | None = None) -> None:
        now = time.monotonic()
        with self._lock:
            stale = (now - self._t_set) > self.STALE_GAP if self._t_set > 0 else True
            if stale or self._target_pos is None:
                # Fresh segment: no interpolation yet.
                self._prev_pos = pos.copy()
                self._target_pos = pos.copy()
                if ori is not None:
                    self._prev_ori = ori.copy()
                    self._target_ori = ori.copy()
            else:
                # Carry the current interpolated value forward as the start
                # of the next segment so the output stays continuous.
                cur_pos = self._interp_pos(now)
                cur_ori = self._interp_ori(now)
                self._prev_pos = cur_pos if cur_pos is not None else pos.copy()
                self._target_pos = pos.copy()
                if ori is not None:
                    self._prev_ori = cur_ori if cur_ori is not None else ori.copy()
                    self._target_ori = ori.copy()
            dt = now - self._t_set if self._t_set > 0 else 0.0
            if 0.0 < dt < self.STALE_GAP:
                self._interval = self.EMA_ALPHA * self._interval + (1.0 - self.EMA_ALPHA) * dt
            self._t_set = now
    ...
    def _alpha(self, now: float) -> float:
        if self._interval <= 1e-6:
            return 1.0
        return min(max((now - self._t_set) / self._interval, 0.0), 1.0)

    def _interp_pos(self, now: float) -> np.ndarray | None:
        if self._target_pos is None:
            return None
        if self._prev_pos is None:
            return self._target_pos
        a = self._alpha(now)
        return self._prev_pos + a * (self._target_pos - self._prev_pos)

    def _interp_ori(self, now: float) -> np.ndarray | None:
        if self._target_ori is None:
            return None
        if self._prev_ori is None:
            return self._target_ori
        a = self._alpha(now)
        # SLERP via axis-angle scaling.
        R_rel = self._target_ori @ self._prev_ori.T
        rv = matrix_to_rotvec(R_rel) * a
        return rotvec_to_matrix(rv) @ self._prev_ori
    ...
    def run(self) -> None:
        while not self._stop.is_set():
            with self._lock:
                ...
                now = time.monotonic()
                interp_pos = self._interp_pos(now) if enabled else None
                interp_ori = self._interp_ori(now) if enabled else None
                tau = self.smooth_tau

            if interp_pos is None:
                # Not streaming -- clear PT1 state so next enable starts fresh.
                self._pub_pos = None
                self._pub_ori = None
                self._t_pub = 0.0
            ...
            # PT1 smoothing stage layered on top of linear interpolation.
            # Rounds the velocity-vector corners at each MediaPipe waypoint.
            if self._pub_pos is None or tau <= 0:
                pub_pos = interp_pos.copy()
                pub_ori = interp_ori.copy() if interp_ori is not None else None
            else:
                dt = (now - self._t_pub) if self._t_pub > 0 else self.period
                dt = max(min(dt, 5 * self.period), 1e-3)
                a = 1.0 - math.exp(-dt / tau)
                pub_pos = self._pub_pos + a * (interp_pos - self._pub_pos)
                if interp_ori is not None and self._pub_ori is not None:
                    R_rel = interp_ori @ self._pub_ori.T
                    rv = matrix_to_rotvec(R_rel) * a
                    pub_ori = rotvec_to_matrix(rv) @ self._pub_ori
                else:
                    pub_ori = interp_ori
            self._pub_pos = pub_pos
            self._pub_ori = pub_ori
            self._t_pub = now

            try:
                self.r.set(self.k_pos, json.dumps(pub_pos.tolist()))
                if pub_ori is not None:
                    self.r.set(self.k_ori, json.dumps(pub_ori.tolist()))
            except Exception as e:   # pragma: no cover -- best-effort streaming
                print(f"[publisher] redis write failed: {e}")
            self._stop.wait(self.period)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--pub-rate", type=float, default=PUB_RATE_HZ,
                   help=f"background publisher rate in Hz (default {PUB_RATE_HZ})")
    p.add_argument("--smooth-tau", type=float, default=SMOOTH_TAU,
                   help=f"PT1 time constant (s) layered on top of interpolation; "
                        f"0 disables, higher=smoother/laggier (default {SMOOTH_TAU})")
...
def main() -> None:
    ...
    WINDOW = "Suturebot webcam teleop (REAL: point/pinch + home + locks + smooth)"
    ...
    # Background publisher: writes interpolated goals to Redis at args.pub_rate.
    publisher = TargetPublisher(r, K_GOAL_POS, K_GOAL_ORI,
                                rate_hz=args.pub_rate, smooth_tau=args.smooth_tau)
    publisher.start()

    print(f"REAL ROBOT teleop (smooth publish). Redis {args.host}:{args.port}, robot={ROBOT_NAME}")
    ...
    print(f"  publisher rate = {args.pub_rate:.0f} Hz   PT1 smooth tau = {args.smooth_tau*1000:.0f} ms")
    print("Keys: 'e' engage | point=move+rotate | pinch=toggle grip | 'h' home | 'r' recenter")
    print("      x/y/z = lock translation    X/Y/Z = lock rotation (roll/pitch/yaw)")
    print("      'q' quit")
    ...
    pinched = False
    grip_closed = False
    lock_pos = np.array([False, False, False])
    lock_rot = np.array([False, False, False])
    ...
    def reanchor(wrist, bbox_diag, R_hand_now) -> bool:
        ...
        nonlocal R_hand_ref, R_robot_ref
        ...
        # Re-seed publisher so interpolation starts from the actual pose.
        publisher.reset()
        publisher.update(anchor_pos, R_robot_ref)
        if cur_ori is not None:
            r.set(K_GOAL_ORI, json.dumps(R_robot_ref.tolist()))
        return True

    def go_home() -> None:
        nonlocal engaged, active, last_pos
        if home_pos is None:
            print("No home pose to return to.")
            return
        # Stop the publisher first so it doesn't overwrite the direct home write.
    ...
    try:
        while True:
            ...
            # Translation locks: zero offset on locked axes.
            ...
            should_stream = engaged and active and hand_seen
            if should_stream:
                publisher.update(target_pos, R_goal_ori)
                last_pos = target_pos
            publisher.enable(should_stream)
