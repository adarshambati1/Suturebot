"""Webcam hand teleop + RealSense view in the SAME window.

Exactly the same controls and behavior as
webcam_hand_publisher_real_point_home_locks_smooth.py (clutched point/pinch
teleop with home, axis locks, and a threaded smoothing publisher), with the
RealSense color stream pulled from Redis displayed side-by-side with the
webcam panel so you can watch the robot end-effector camera while you
teleoperate.

Architecture:
  * Teleop runs on the Mac as before -- owns the laptop webcam, writes goals
    through the SSH tunnel via the background publisher.
  * RealSense frames come from python_examples/realsense_publisher.py running
    on the Linux PC, which JPEG-encodes and SETs them to a Redis key
    (default: suturebot::realsense::color). This script reads that key each
    main-loop tick using the SAME Redis connection / SSH tunnel.
  * Webcam panel (with hand landmarks + HUD) is on the LEFT; RealSense panel
    is on the RIGHT. If the publisher isn't running, the right panel shows a
    "waiting" placeholder and the teleop still works.

Controls (same as the smooth file):
...
Usage:
    # On the PC, in another terminal:
    python python_examples/realsense_publisher.py
    # On the Mac:
    python vr_teleop/test_harness/webcam_hand_publisher_real_point_home_locks_smooth_rs.py --port 6380
"""
...
REALSENSE_KEY_DEFAULT = "suturebot::realsense::color"
...
def label_rs_panel(img: np.ndarray, fps: float, lag_ms: float, alive: bool) -> None:
    """Banner with name + fps + lag on the RealSense panel."""
    cv2.rectangle(img, (0, 0), (img.shape[1], 28), (30, 30, 30), -1)
    cv2.putText(img, "RealSense (PC)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.55,
                (255, 255, 255), 1, cv2.LINE_AA)
    if alive:
        right = f"{fps:4.1f} fps   lag {lag_ms:4.0f} ms"
        cv2.putText(img, right, (max(0, img.shape[1] - 220), 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
    else:
        cv2.putText(img, "(waiting)", (max(0, img.shape[1] - 110), 20),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--smooth-tau", type=float, default=SMOOTH_TAU,
                   help=f"PT1 time constant (s) on top of interpolation (default {SMOOTH_TAU})")
    p.add_argument("--realsense-key", default=REALSENSE_KEY_DEFAULT,
                   help=f"Redis key the RealSense publisher writes to "
                        f"(default: {REALSENSE_KEY_DEFAULT})")
    p.add_argument("--no-realsense", action="store_true",
                   help="Don't display the RealSense panel (webcam only)")
...
def main() -> None:
    ...
    WINDOW = "Suturebot teleop + RealSense"
    cv2.namedWindow(WINDOW, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(WINDOW, 1600, 540)
    ...
    print(f"REAL ROBOT teleop + RealSense. Redis {args.host}:{args.port}, robot={ROBOT_NAME}")
    ...
    print(f"  RealSense {'ON (key %s)' % args.realsense_key if rs_on else 'OFF'}")
    ...
    # RealSense panel state (decode once per new frame; cache otherwise).
    rs_panel_cached: np.ndarray | None = None
    rs_last_ts = -1.0
    rs_frames_seen = 0
    rs_t_fps = time.monotonic()
    rs_fps = 0.0
    rs_lag_ms = 0.0
    ...
    try:
        while True:
            ...
            # ---- Build the combined display: webcam panel + RealSense panel ----
            if rs_on:
                rs_data = r.get(args.realsense_key)
                if rs_data is not None and len(rs_data) >= 8:
                    pub_ts = struct.unpack("<d", rs_data[:8])[0]
                    if pub_ts != rs_last_ts:
                        arr = np.frombuffer(rs_data[8:], dtype=np.uint8)
                        decoded = cv2.imdecode(arr, cv2.IMREAD_COLOR)
                        if decoded is not None:
                            rs_panel_cached = decoded
                            rs_frames_seen += 1
                        rs_last_ts = pub_ts
                    rs_lag_ms = (time.time() - pub_ts) * 1000.0
                now = time.monotonic()
                if now - rs_t_fps >= 1.0:
                    rs_fps = rs_frames_seen / (now - rs_t_fps)
                    rs_frames_seen = 0
                    rs_t_fps = now

                fh = frame.shape[0]
                if rs_panel_cached is not None:
                    rs_show = fit_height(rs_panel_cached, fh).copy()
                    label_rs_panel(rs_show, rs_fps, rs_lag_ms, alive=True)
                else:
                    rs_show = np.zeros((fh, max(1, fh * 4 // 3), 3), dtype=np.uint8)
                    cv2.putText(rs_show, "waiting for RealSense publisher...",
                                (20, fh // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.55,
                                (180, 180, 180), 1, cv2.LINE_AA)
                    label_rs_panel(rs_show, 0.0, 0.0, alive=False)
                combined = np.hstack([frame, rs_show])
            else:
                combined = frame
