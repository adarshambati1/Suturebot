"""Webcam hand teleop + RealSense 2D + RealSense 3D point cloud in one window.

Superset of webcam_hand_publisher_real_point_home_locks_smooth_rs.py: adds a
third panel that displays the textured 3D point-cloud render published by
python_examples/realsense_publisher.py to the suturebot::realsense::pointcloud
key. Equivalent to picking the "3D" tab in realsense-viewer, but rendered on
the PC, JPEG-streamed over the tunnel, and laid out next to the webcam and
2D color panels.

Layout: [ Webcam (Mac) | RealSense color (PC) | RealSense 3D (PC) ]

The 2D and 3D panels each show fps + lag; both fall back to a "waiting"
placeholder if the matching publisher key isn't being written.

Controls (same as the smooth_rs file):
...
Usage:
    # On the PC (publishes color + 3D point cloud):
    python python_examples/realsense_publisher.py
    # On the Mac:
    python vr_teleop/test_harness/webcam_hand_publisher_real_point_home_locks_smooth_rs3d.py --port 6380
"""
...
REALSENSE_COLOR_KEY_DEFAULT = "suturebot::realsense::color"
REALSENSE_3D_KEY_DEFAULT    = "suturebot::realsense::pointcloud"
...
def label_remote_panel(img: np.ndarray, title: str, fps: float, lag_ms: float, alive: bool) -> None:
...
def make_waiting_panel(text: str, h: int) -> np.ndarray:
    """Black placeholder panel of height h, default 4:3 aspect."""
    img = np.zeros((h, max(1, h * 4 // 3), 3), dtype=np.uint8)
    cv2.putText(img, text, (20, h // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.55,
...
class TargetPublisher(threading.Thread):
    ...
    def __init__(self, r, k_pos, k_ori, rate_hz, smooth_tau=SMOOTH_TAU):
    ...
    def update(self, pos, ori=None) -> None:
        now = time.monotonic()
        with self._lock:
...
class RemotePanelReader:
    """Polls Redis for a JPEG-bytes-with-timestamp-header panel and caches the
    latest decoded frame, tracking fps and end-to-end lag."""

    def __init__(self, r: redis.Redis, key: str, title: str):
        self.r = r
        self.key = key
        self.title = title
        ...
        if data is not None and len(data) >= 8:
            ...
            if pub_ts != self.last_ts:
                ...
                if decoded is not None:
                    ...
                    self.frames_seen += 1
                self.last_ts = pub_ts
            self.lag_ms = (time.time() - pub_ts) * 1000.0
        now = time.monotonic()
        if now - self.t_fps >= 1.0:
            self.fps = self.frames_seen / (now - self.t_fps)
            self.frames_seen = 0
            self.t_fps = now

    def render(self, target_h: int) -> np.ndarray:
        if self.cached is None:
            panel = make_waiting_panel(f"waiting for {self.title}...", target_h)
            label_remote_panel(panel, self.title, 0.0, 0.0, alive=False)
            return panel
        panel = fit_height(self.cached, target_h).copy()
        label_remote_panel(panel, self.title, self.fps, self.lag_ms, alive=True)
        return panel
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--realsense-color-key", default=REALSENSE_COLOR_KEY_DEFAULT,
                   help=f"Redis key for the color stream (default: {REALSENSE_COLOR_KEY_DEFAULT})")
    p.add_argument("--realsense-3d-key", default=REALSENSE_3D_KEY_DEFAULT,
                   help=f"Redis key for the 3D pointcloud render (default: {REALSENSE_3D_KEY_DEFAULT})")
    p.add_argument("--no-realsense", action="store_true",
                   help="Hide both RealSense panels")
    p.add_argument("--no-3d", action="store_true",
                   help="Hide just the 3D point cloud panel")
...
def main() -> None:
    ...
    rs3d_on = rs_on and not args.no_3d
    ...
    WINDOW = "Suturebot teleop + RealSense 2D + RealSense 3D"
    cv2.namedWindow(WINDOW, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(WINDOW, 1800, 540)
    ...
    rs_color = RemotePanelReader(r, args.realsense_color_key, "RealSense color") if rs_on else None
    rs_3d    = RemotePanelReader(r, args.realsense_3d_key,    "RealSense 3D")    if rs3d_on else None

    print(f"REAL ROBOT teleop + RealSense 2D + 3D. Redis {args.host}:{args.port}, robot={ROBOT_NAME}")
    ...
    if rs_on:
        print(f"  RealSense color key '{args.realsense_color_key}'")
    if rs3d_on:
        print(f"  RealSense 3D    key '{args.realsense_3d_key}'")
    ...
    try:
        while True:
            ...
            if rs_color is not None:
                rs_color.poll()
                panels.append(rs_color.render(target_h))
            if rs_3d is not None:
                rs_3d.poll()
                panels.append(rs_3d.render(target_h))
